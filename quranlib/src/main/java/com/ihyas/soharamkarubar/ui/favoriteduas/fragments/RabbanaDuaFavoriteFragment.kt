...
import com.ihyas.soharamkaru.databinding.FragmentRabbanaDuaFavoriteBinding
import com.ihyas.soharamkarubar.download.DownloadFavoriteRabbanaDuaAudioFile
...
class RabbanaDuaFavoriteFragment : Fragment() {

    lateinit var binding: FragmentRabbanaDuaFavoriteBinding
    val duasViewModel: DuasViewModel by hiltNavGraphViewModels(R.id.graph_allduas)
    ...
    private val server_path = "http://www.muslimguidepro.com/data/Rabbana%20Dua/"
    private val fileDirectory = "MuslimGuideProRabbana"
    val args: RabbanaDuaFavoriteFragmentArgs by navArgs()
    ...
    override fun onCreateView(
        ...
        binding = FragmentRabbanaDuaFavoriteBinding.inflate(inflater)
        return binding.root
    }
    ...
    private fun initUtils(context: Context) {
        ...
        viewLifecycleOwner.lifecycleScope.launch(IO) {
            ...
            favoriteDuasList.forEach {
                indexes.add(it.title.split("Rabbana Duas ")[1].toInt() - 1)
            }
            if (indexes.size == 1) {
                withContext(Main) {
            ...
            when (args.index) {
                favoriteDuasList[0].title.split("Rabbana Duas ")[1].toInt() - 1 ->
                    withContext(Main) {
                        binding.include.rabbanaPreBtn.setImageResource(R.drawable.ic_prev_disabled)
                        binding.include.rabbanaPreBtn.isEnabled = false
                    }
                favoriteDuasList[favoriteDuasList.lastIndex].title.split("Rabbana Duas ")[1].toInt() - 1 ->
                    withContext(Main) {
                        binding.include.rabbanaNextBtn.setImageResource(R.drawable.ic_next_disabled)
                        binding.include.rabbanaNextBtn.isEnabled = false
                    }
    ...
    private fun onClickEvents(context: Context) {
        ...
        binding.include.rabbanaPreBtn.setSafeOnClickListener { v: View? -> showPreviousSurah() }
        binding.include.rabbanaNextBtn.setSafeOnClickListener { v: View? -> showNextSurah() }
        ...
        binding.include.seekBar1.setOnSeekBarChangeListener(object :
            SeekBar.OnSeekBarChangeListener {
                ...
                try {
                    if (InternetManager.checkForInternet(context)) {
                        if (f.createNewFile()) {
                            val audio =
                                activity?.let {
                                    DownloadFavoriteRabbanaDuaAudioFile(
                                        it,
                                        this@RabbanaDuaFavoriteFragment
                                    )
                                }
                        ...
                        Toast.makeText(context,
                            getString(R.string.internet_not_available), Toast.LENGTH_SHORT).show()
                    }
                } catch (e: Exception) {
                    e.printStackTrace()
                }
        ...
        binding.include25.btnMore.setSafeOnClickListener { v: View? ->
            ...
            val shareBody = """
                ...
                $ayahTranslation            
                $footer              
                Get the Islamic Guide Pro to explore more Islamic content:
        ...
        binding.include25.btnDelete.setSafeOnClickListener {
            viewLifecycleOwner.lifecycleScope.launch(IO) {
                var favDua = duasViewModel.getDuaSaved("Rabbana Duas ${currentDua + 1}")
                if (favDua != null) {
                    withContext(Main) {
                        duasViewModel.deleteDua(favDua)
    ...
    private fun showPreviousSurah() {
        var i = indexes.indexOf(currentDua)
        if (i > 0) {
            currentDua = indexes[--i]
            loadDataFromFile()
        }
        if (player != null) {
            player?.stop()
        ...
        if (indexes.indexOf(currentDua) == 0) {
            binding.include.rabbanaPreBtn.setImageResource(R.drawable.ic_prev_disabled)
            binding.include.rabbanaPreBtn.isEnabled = false
...
//        animateToSoundSettings(false)
    ...
    private fun showNextSurah() {
        ...
        if (indexes.indexOf(currentDua) == indexes.size - 1) {
            binding.include.rabbanaNextBtn.setImageResource(R.drawable.ic_next_disabled)
            binding.include.rabbanaNextBtn.isEnabled = false
            binding.include.rabbanaPreBtn.isEnabled = true
        } else {
...
//        animateToSoundSettings(false)
    ...
    private fun setUpFavoriteButton() {
        viewLifecycleOwner.lifecycleScope.launch(IO) {
            if (duasViewModel.getDuaSaved("Rabbana Duas ${currentDua + 1}") != null) {
                withContext(Main) {
                    binding.include25.btnDelete.setImageDrawable(
                        resources.getDrawable(
                            R.drawable.ic_ins_favorite
